...
This skill documents the official `hey` CLI (v1.4.0). In EmCP, call the matching
`hey_*` tools instead of shelling out. Command families map 1:1:

| CLI | MCP tools |
| --- | --- |
| `hey box list` / `hey box view` | `hey_boxes`, `hey_box` |
| `hey search` / `hey search filters` | `hey_search`, `hey_search_filters` |
| `hey thread read` | `hey_threads` |
| `hey label` / `hey collection` / `hey workflow` | `hey_labels`, `hey_label_*`, `hey_collections`, `hey_collection_*`, `hey_workflows`, `hey_workflow_*` |
| `hey contact` / `hey screener` | `hey_contacts`, `hey_contact_*`, `hey_screener*` |
| `hey compose` / `hey reply` / `hey forward` / `hey draft` | `hey_compose`, `hey_reply`, `hey_forward`, `hey_drafts`, `hey_draft*` |
| `hey calendar list` / `hey event` | `hey_calendars`, `hey_events`, `hey_event_day`, `hey_event_week`, `hey_event_*` |
| `hey todo` / `hey habit` / `hey timetrack` / `hey journal` | `hey_todo_*`, `hey_habits`, `hey_habit_*`, `hey_timetrack_*`, `hey_journal_*` |

**IDs:** use box item `id` for seen/move/label/trash/spam/ignore/bubble. Use `topic_id` for thread read/reply/forward/share/collection/workflow/attachments.

**Bodies:** prefer the `paragraphs` array on compose/reply/forward (Markdown). The CLI converts Markdown to HEY rich text. Use `message_html` or `as_html` only when you must send HTML.

**Accounts:** pass `account` on mail tools, or call `hey_accounts` / `hey_account_use`.
